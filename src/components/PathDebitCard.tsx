...
export default function PathDebitCard({ stats, onUpdateStats, onFinish }: PathDebitCardProps) {
  ...
  const performCheckout = () => {
    ...
    if (checkingBal >= totalGroceryCost) {
      ...
      if (savingsBal >= overdraftAmount) {
        ...
        // Leo overdrafts! Overdraft bank fees hit
  ...
  const handleResolveEmergency = () => {
    ...
    // Leo will spend from Savings first, then checking
    if (savingsBal >= cost) {
    ...
    <div className="max-w-xl mx-auto px-4 py-8 space-y-6">
      ...
      {step === 'teller' && (
        ...
        >
          ...
          <p className="text-slate-300 text-sm leading-relaxed">
            Leo stands at the marble counter of the <strong>Local Credit Union</strong>. Clara, the friendly teller, smiles warmly.
          </p>
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs italic text-slate-400 relative">
            ...
            "Welcome, Leo! First week wages? Excellent work! Let's get that cash sorted. Would you like a Checking account for daily transactions (Debit card), a Savings account to keep money safe and earn a trickle of compound interest, or should we open both for a balanced budget?"
          </div>
          ...
          <div className="space-y-3 pt-2">
            <button
              ...
              className="w-full p-4 rounded-xl border border-slate-800 hover:border-indigo-500 hover:bg-slate-800/40 text-left transition-all cursor-pointer group space-y-1"
            >
              <div className="font-bold text-slate-200 group-hover:text-indigo-400 text-sm flex items-center gap-2">
                <span>Checking Account Only</span>
                <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 bg-indigo-950 text-indigo-400 rounded">Option A</span>
              </div>
              <p className="text-xs text-slate-400">
                Put all $500 in Checking. Access everything instantly with your new shiny Debit Card. Savings balance remains $0.
              </p>
            </button>
            ...
            <button
              ...
              className="w-full p-4 rounded-xl border border-slate-800 hover:border-amber-500 hover:bg-slate-800/40 text-left transition-all cursor-pointer group space-y-1"
            >
              <div className="font-bold text-slate-200 group-hover:text-amber-400 text-sm flex items-center gap-2">
                <span>Savings Account Only</span>
                <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 bg-amber-950 text-amber-500 rounded">Option B</span>
              </div>
              <p className="text-xs text-slate-400">
                Put all $500 in Savings. Safest way to lock it up for the laptop, but hard to access for quick daily needs.
              </p>
            </button>
            ...
            <button
              ...
              className="w-full p-4 rounded-xl border border-emerald-500/40 hover:border-emerald-500 hover:bg-slate-800/40 text-left transition-all cursor-pointer group space-y-1 bg-emerald-950/5"
            >
              <div className="font-bold text-slate-100 group-hover:text-emerald-400 text-sm flex items-center gap-2">
                <span>Checking & Savings Account Combo</span>
                <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 bg-emerald-950 text-emerald-400 rounded">OPTION C (Recommended)</span>
              </div>
              <p className="text-xs text-slate-400">
                Split of <strong>$400 in Checking / $100 in Savings</strong>. Earn minor interest on savings, use debit card for daily necessities up to $400.
              </p>
            </button>
          </div>
        </motion.div>
      ...
      {step === 'g_intro' && (
        ...
        >
          ...
          <p className="text-slate-300 text-sm leading-relaxed">
            Leo leaves the bank holding a sleek ocean-blue Debit Card. Now, he must head to <strong>Apex Foods & Tech</strong> to stock up for the upcoming college semester.
          </p>

          <p className="text-slate-300 text-sm leading-relaxed">
            He has a <strong>grocery limit of $100</strong>. Items will pass by on an automated checkout conveyor belt. Leo must classify them carefully.
          </p>
      ...
      {step === 'g_checkout' && (
        ...
        >
          ...
          <p className="text-slate-300 text-sm leading-relaxed">
            Leo places his items at the register. The cashier rings up the grocery haul.
          </p>
      ...
      {step === 'emergency' && (
        ...
        >
          ...
          <p className="text-slate-300 text-sm max-w-sm mx-auto leading-relaxed">
            While holding his heavy backpack, Leo accidentally drops his smartphone onto the asphalt concrete! The display is completely shattered.
          </p>
          ...
          >
            How will Leo pay this unexpected bill?
          </button>
        </motion.div>
      )}
      ...
      {step === 'emergency_resolution' && (
        ...
        >
          ...
          <p className="text-slate-300 text-sm leading-relaxed">
            Life doesn't wait for your checking account or your budgets to line up. Leo must pay <strong>$150</strong> right now to get his communication device restored.
          </p>
            ...
            <div className="pt-2 italic text-slate-400 leading-normal">
              {savingsBal >= 100 ? (
                <span className="text-emerald-400 font-medium">
                  ⭐️ Shield Activated! Leo's dedicated $100 Savings acted as an Emergency Fund. He only struggles to find $50 from his remaining checking cash to fully cover the repair. No debt, no stress penalties!
                </span>
              ) : (
                <span className="text-rose-400 font-medium">
                  ⚠️ No Emergency Fund! All his money was in checking (and some spent on Wants), or locked totally in savings with zero liquid cash. Leo is short on funds to fix his phone. He must acquire outstanding loans and drops overall happiness.
                </span>
              )}
            </div>
          </div>
